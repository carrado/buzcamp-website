	...
	var update = function(){
	 ...
	 // call update initially, in case input is pre-filled
		update();
          ...
          $.ajax({
           type: "POST",
           url: url+"electronicsproduct/electronicsupload",
           data: $(this).serialize(),
           beforeSend: function()
           {
         ...
         success: function(data)
          {
            if(data != "")
            {
                   $(".loading").hide();
                      ...
                      if((data == '#kindsselect') || (data == '#salesselect'))
                      {
                        $(data).fadeIn();
                      }
                     else if((data != '#kindsselect') || (data != '#salesselect'))
                       {
          ...
          $(document).on("ready", function() {
            var cover = 1;
              $("#uploadBtn3").fileinput({
                  uploadAsync: false,
                 showUpload:false,
                  uploadUrl: url+"electronicsproduct/electronicsphoto", // this is actually not important
              showBrowse: false,
              showCaption: false,
              browseOnZoneClick: true,
              dropZoneClickTitle: "<img src="+base_url+"img/e1.png>",
              });
          });
