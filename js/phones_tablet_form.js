	...
	var update = function(){
	 ...
	 // call update initially, in case input is pre-filled
		update();
          ...
          {
            $("#phonestabform").submit(function()
            {
            var name = $('input[name=name]');
            var brand = $('input[name=brand]');
            var ram = $('input[name=ram]');
            var os = $('input[name=os]');
            var cpu = $('input[name=cpu]');
            var battery = $('input[name=battery]');
            var resolution = $('input[name=resolution]');
            var camera = $('input[name=camera]');
          ...
          if(ram.val()==''){
            ram.addClass('highlight');
            return false;
          }else ram.removeClass('highlight');

          if(os.val()==''){
            os.addClass('highlight');
            return false;
          }else os.removeClass('highlight');

          if(cpu.val()==''){
            cpu.addClass('highlight');
            return false;
          }else cpu.removeClass('highlight');

          if(battery.val()==''){
            battery.addClass('highlight');
            return false;
          }else battery.removeClass('highlight');

          if(resolution.val()==''){
            resolution.addClass('highlight');
            return false;
          }else resolution.removeClass('highlight');

          if(camera.val()==''){
            camera.addClass('highlight');
            return false;
          }else camera.removeClass('highlight');
          ...
          if(price.val()==''){
            ...
            $.ajax({
             type: "POST",
             url: url+"phonestabproduct/phonestabupload",
             data: $(this).serialize(),
             beforeSend: function()
             {
            ...
            $(document).on("ready", function() {
              var cover = 1;
                $("#uploadBtn3").fileinput({
                    uploadAsync: false,
                   showUpload:false,
                    uploadUrl: url+"phonestabproduct/phonestabphoto", // this is actually not important
                showBrowse: false,
                showCaption: false,
                browseOnZoneClick: true,
                dropZoneClickTitle: "<img src="+base_url+"img/p1.jpg>",
                });
            });
